...
public class UserServiceImpl implements IUserService{
	...
	@Override
	public boolean validateUser(int userId,String userName, String userPassword) throws UserNotFoundException {
		Optional<User> opt=userRepository.findById(userId);
		if(opt.isPresent()) {
			User existingUser = opt.get();
			if(existingUser.getUserName().equals(userName)&&existingUser.getUserPassword().equals(userPassword))
				return true;
			else
				return false;
		} else {
